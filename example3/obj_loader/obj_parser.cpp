...
void obj_set_material_defaults(obj_material *mtl)
{
	...
	mtl->reflect = 0.0f;
	mtl->trans = 1;
	mtl->glossy = 1000; //default is really 60
	mtl->shiny = 0;
	mtl->refract_index = 1;
...
obj_face* obj_parse_face(obj_growable_scene *scene)
{
	int vertex_count;
...
obj_sphere* obj_parse_sphere(obj_growable_scene *scene)
{
...
obj_plane* obj_parse_plane(obj_growable_scene *scene)
{
	int temp_indices[MAX_VERTEX_COUNT];
...
obj_light_point* obj_parse_light_point(obj_growable_scene *scene)
{
...
obj_light_quad* obj_parse_light_quad(obj_growable_scene *scene)
{
...
obj_light_disc* obj_parse_light_disc(obj_growable_scene *scene)
{
	int temp_indices[MAX_VERTEX_COUNT];
...
obj_vector* obj_parse_texture_vertex()
{
	obj_vector *v = (obj_vector*)malloc(sizeof(obj_vector));
	v->e[0] = (float) atof( strtok(NULL, WHITESPACE));
	v->e[1] = (float) atof( strtok(NULL, WHITESPACE));
...
void obj_parse_camera(obj_growable_scene *scene, obj_camera *camera)
{
	int indices[3];
...
int obj_parse_obj_file(obj_growable_scene *growable_data, char *filename)
{
...
/*		
	extreme_dimensions[0].c[0] = INFINITY; extreme_dimensions[0].c[1] = INFINITY; extreme_dimensions[0].c[2] = INFINITY;
	extreme_dimensions[1].c[0] = -INFINITY; extreme_dimensions[1].c[1] = -INFINITY; extreme_dimensions[1].c[2] = -INFINITY;

			if(v.c[0] < extreme_dimensions[0].c[0]) extreme_dimensions[0].c[0] = v.c[0];
			if(v.c[0] > extreme_dimensions[1].c[0]) extreme_dimensions[1].c[0] = v.c[0];
			if(v.c[1] < extreme_dimensions[0].c[1]) extreme_dimensions[0].c[1] = v.c[1];
			if(v.c[1] > extreme_dimensions[1].c[1]) extreme_dimensions[1].c[1] = v.c[1];
			if(v.c[2] < extreme_dimensions[0].c[2]) extreme_dimensions[0].c[2] = v.c[2];
			if(v.c[2] > extreme_dimensions[1].c[2]) extreme_dimensions[1].c[2] = v.c[2];*/
	...
	while( fgets(current_line, OBJ_LINE_SIZE, obj_file_stream) )
	{
		...
		{
			list_add_item(&growable_data->vertex_texture_list,  obj_parse_texture_vertex(), NULL);
		}
...
#define MAX_PATH_LENGTH 1000
			...
			int filenameSize = sizeof(growable_data->material_filename);
			int pathSize = sizeof(objPath.path);
			char *mtlPath = (char*) malloc(filenameSize + pathSize + 1);
			strncpy(mtlPath, objPath.path, filenameSize);
			strncat(mtlPath, growable_data->material_filename, filenameSize + pathSize + 1);
			
			printf("%s\n", mtlPath);
			obj_parse_mtl_file(mtlPath, &growable_data->material_list);
		...
		//else if( strequal(current_token, "o") ) //Primitive name
		//{ }
...
void obj_init_temp_storage(obj_growable_scene *growable_data)
{
...
void obj_free_temp_storage(obj_growable_scene *growable_data)
{
...
void delete_obj_data(obj_scene *data_out)
{
	int i;
...
void obj_copy_to_out_storage(obj_scene *data_out, obj_growable_scene *growable_data)
{
...
int parse_obj_scene(obj_scene *data_out, char *filename)
{
	obj_growable_scene growable_data;
